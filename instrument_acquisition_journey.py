...
ACQ_SELECT = (
    ...
    "urs_reference,urs_approval_date,quotation_reference,quotation_date,pr_number,pr_approval_date,"
...
def _milestones(inst):
    return [
        ...
        },
        {
            ...
            "next": "Record the selected supplier quotation before raising the Purchase Requisition (PR).",
        },
        {
            "stage": "3. PR Approved",
            "reference": _ref_text(inst.get("pr_number")),
            "date": inst.get("pr_approval_date"),
            "done": bool(_ref_text(inst.get("pr_number")) and inst.get("pr_approval_date")),
            "next": "Record the Purchase Requisition (PR) number and approval date.",
        },
        {
            "stage": "4. PO Approved",
            ...
            "next": "Record the Purchase Order (PO) number and approval date.",
        },
        {
            ...
            "next": "Set the expected receiving date and monitor the supplier commitment.",
        },
        {
            "stage": "6. Received",
            "reference": "",
        ...
        },
        {
            ...
            "next": "Record the first approved / routine analytical run after IQ, OQ, and PQ.",
        },
    ]
...
def _chronology_warnings(inst):
    ...
    for name, raw in ordered:
        ...
        if current and previous_date and current < previous_date:
            warnings.append(
                f"{name} ({current}) is earlier than {previous_name} ({previous_date}). "
                "Review the sequence or the entered date."
            )
        if current:
...
st.header("URS · PR · PO | Acquisition & Commissioning Journey")
...
**URS → Quotation → PR → PO → Expected Receiving → Receiving → Issuance → IQ → OQ → PQ → First Run**

Why it matters:
- exposes procurement or approval delays before they become surprises,
- preserves the link between the approved requirement, purchasing process, and the actual instrument,
- makes commissioning readiness visible to QC leadership,
- separates **expected** receiving from **actual** receiving,
- prevents an instrument from appearing operational before IQ/OQ/PQ and first-run readiness are documented,
- creates evidence that can later support lifecycle reviews and investigation context.

**Important:** this application is decision-support and lifecycle-visibility software. Approved URS, PR, PO, qualification protocols/reports, and other official GMP records remain in your controlled systems.

**ببساطة:** إحنا مش بنسجل تاريخ وصول الجهاز فقط؛ إحنا بنشوف الرحلة كاملة من الاحتياج والشراء لحد أول تشغيل فعلي.
"""
    )
...
if not journey_ok:
...
elif not journey_instruments:
    st.info("Create an Instrument Passport first. The acquisition journey is linked to the Instrument ID.")
else:
    options = {
    ...
    st.caption("Workflow: URS → Quotation → PR → PO → Receiving → Qualification → First Run")
    ...
    with st.form(f"acq_form_{inst.get('id')}"):
        ...
        st.markdown("#### 3 · Purchase Requisition (PR)")
        c1, c2 = st.columns(2)
        pr_number = c1.text_input("PR number", value=_ref_text(inst.get("pr_number")))
        pr_date = c2.date_input("PR approval date", value=_journey_date(inst.get("pr_approval_date")))

        st.markdown("#### 4 · Purchase Order (PO)")
        c1, c2 = st.columns(2)
        ...
        c1, c2 = st.columns(2)
        expected_receiving = c1.date_input(
            "Expected receiving date",
            value=_journey_date(inst.get("expected_receiving_date")),
        )
        receiving_date = c2.date_input(
            "Actual receiving date",
            value=_journey_date(inst.get("receiving_date")),
        )

        st.markdown("#### 7 · Issuance")
        issuance_date = st.date_input(
            "Issuance / laboratory handover date",
            value=_journey_date(inst.get("issuance_date")),
        )
        ...
        first_run_date = st.date_input(
            "First approved / routine run date",
            value=_journey_date(inst.get("first_run_date")),
        )
    ...
    if save:
        payload = {
            ...
            "pr_number": pr_number.strip() or None,
            "pr_approval_date": _iso_or_none(pr_date),
...
2. Start with the **URS reference + URS approval date**.
3. Record the selected **Quotation**.
4. Record the **PR number + PR approval date**, followed by **PO number + PO approval date**.
5. Enter both **Expected Receiving** and **Actual Receiving** so delays become visible.
6. After receipt, record **Issuance → IQ → OQ → PQ**.
7. Add the **First Run date** only after the qualification sequence is complete.
8. Use progress and **Recommended next action** as management signals — never as substitutes for controlled records.
"""
    )
    st.markdown(
        '<div class="cta"><b>CTA → Complete the next missing milestone.</b><br>'
        'A complete URS-to-First-Run timeline turns procurement history into usable QC lifecycle evidence.</div>',
        unsafe_allow_html=True,
    )
